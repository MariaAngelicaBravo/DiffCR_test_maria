...
class BaseModel():
    ...
    def load_network(self, network, network_label, strict=True):
        ...
        # support two forms for resume_state:
        # 1) a prefix (e.g. 'pretrained/diffcr_new') -> loads 'pretrained/diffcr_new_<network_label>.pth'
        # 2) a full .pth path (e.g. 'pretrained/diffcr_new.pth') -> loads that file directly
        resume = self.opt['path']['resume_state']
        candidate1 = resume
        candidate2 = "{}_{}.pth".format(resume, network_label)
        ...
        if os.path.exists(candidate1):
            ...
            self.logger.warning('Pretrained model in [{:s}] or [{:s}] does not exist, skip.'.format(candidate1, candidate2))
            return
