...
class Sen2_MTC_New_Multi(data.Dataset):
    ...
    def __init__(self, data_root, mode='train'):
        ...
        if mode == 'train':
            tile_list = np.loadtxt(os.path.join(data_root, 'train.txt'), dtype=str)
        elif mode == 'val':
            tile_list = np.loadtxt(os.path.join(data_root, 'val.txt'), dtype=str)
        elif mode == 'test':
            tile_list = np.loadtxt(os.path.join(data_root, 'test.txt'), dtype=str)

        for tile in tile_list:

            cloud_dir = os.path.join(data_root, tile, 'cloud')
            cloudless_dir = os.path.join(data_root, tile, 'cloudless')

            if not os.path.exists(cloud_dir) or not os.path.exists(cloudless_dir):
                continue

            # leer imágenes cloud ordenadas (01,02,03)
            cloud_images = sorted([
            ...
            # leer GT
            cloudless_images = sorted([
                os.path.join(cloudless_dir, f)
                for f in os.listdir(cloudless_dir)
                if f.endswith(".tif")
            ])

            if len(cloud_images) < 3 or len(cloudless_images) < 1:
                continue

            cloud0 = cloud_images[0]   # fused
            cloud1 = cloud_images[1]   # cloud_1
            cloud2 = cloud_images[2]   # cloud_2
            cloudless = cloudless_images[0]
            ...
            name = os.path.basename(cloudless).replace(".tif", "")
    ...
    def __getitem__(self, index):
        ...
        cloud0_path, cloud1_path, cloud2_path, cloudless_path = self.filepair[index]

        image_cloud0 = self.image_read(cloud0_path)
        image_cloud1 = self.image_read(cloud1_path)
        image_cloud2 = self.image_read(cloud2_path)
        image_cloudless = self.image_read(cloudless_path)

        ret = {}

        ret['gt_image'] = image_cloudless[:3,:,:]

        ret['cond_image'] = torch.cat([
            image_cloud0[:3,:,:],
            image_cloud1[:3,:,:],
            image_cloud2[:3,:,:]
        ])
    ...
    def image_read(self, image_path):
        ...
        mean = torch.tensor([0.5,0.5,0.5,0.5]).view(-1,1,1)
        std = torch.tensor([0.5,0.5,0.5,0.5]).view(-1,1,1)
...
class Sen2_MTC_New1(data.Dataset): # esta clase es similar a la anterior pero en lugar de concatenar las 3 imágenes con nubes, se elige una al azar para usarla como imagen condicional
    def __init__(self, data_root, mode='train'):
        self.data_root = data_root
        self.mode = mode
        self.filepair = []
...
# esta clase es similar a la anterior pero en lugar de elegir una imagen con nubes al azar, se concatenan 2 imágenes con nubes para usarlas como imagen condicional
...
# esta clase es para tareas de inpainting, uncropping y colorization, donde se generan máscaras de diferentes formas para ocultar partes de la imagen y se usan como condición para la generación de la imagen completa
...
if __name__=='__main__':
    ...
    for ret in Sen2_MTC_New_Multi(
        "/mnt/compartida/mbravo_cps/DOCTORADO/DiffCR_results/data/NEW_DATASET",
        "val"
    ):
